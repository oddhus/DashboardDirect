...
getCompleteTable <- function() {
  ...
  implantData <- implant %>%
    ...
    rename(ExtractionReason = Name) %>%
    select(-ExtractionReasonId) %>%
  ...
  antibioticsData <- antibioticsRelation %>%
    ...
    rename(AntibioticName = Name) %>%
    select(-AntibioticsId)

  implantType <- tbl(con, "Implants")
  vendor <- tbl(con, "Vendor")

  implantTypeData <- implantType %>%
    rename(ImplantName = Name) %>%
  ...
  insertionData <- patients %>%
    ...
    rename(LekholmZarbVolume = Name) %>%
    select(-LekholmZarbVolumeId) %>%
    left_join(lekholmZarbDensity, by = c("LekholmZarbDensityId" = "Id")) %>%
    rename(LekholmZarbDensity = Name) %>%
    select(-LekholmZarbDensityId) %>%
  ...
  removalData <- patients %>%
    ...
    left_join(implantTypeData, by = c("ImplantsId" = "Id")) %>%
    select(
      PatientId, Position, RemovalId, InsertionDate, RemovalDate, RemovalReason,
  ...
  data <- insertionData %>%
    ...
    mutate(
      ImplantLengthMillimeter = coalesce(ImplantLengthMillimeter.x, ImplantLengthMillimeter.y),
    ...
    mutate(
      ...
      ImplantLengthFactor = case_when(
        ceiling(ImplantLengthMillimeter) < 10 ~ "< 10mm",
        ceiling(ImplantLengthMillimeter) <= 15 ~ "10-15mm",
        ceiling(ImplantLengthMillimeter) > 15 ~ "> 15mm",
        TRUE ~ as.character(NA)
      ),
      ...
      AntibioticsDoseMgFactor = case_when(
        ceiling(AntibioticsDoseMg) < 2000 ~ "< 2500mg",
